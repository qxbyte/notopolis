...
const MAX_CANDIDATES = 300; // 每次散布候选点数（越多间隙越贴近目标值，布局越紧凑）
...
export function layoutDistricts(counts: { dir: string; count: number }[]): Plot[] {
  ...
  // 系数 0.82：散布更紧凑，区群占满地图主体而非漂在大片旷野中
  const SPREAD_R = Math.max(Math.sqrt(total * UNIT_AREA) * 0.82, maxR * 2.1);
  ...
  for (let idx = 0; idx < items.length; idx++) {
    ...
    // 每个团块个体化目标间隙：~4 到 ~14 单位（间隙过大会让地图显空）
    const targetGap_i = TARGET_GAP * (0.5 + rngScatter() * 1.3);
    ...
    if (idx === 0) {
      ...
      for (let c = 0; c < MAX_CANDIDATES; c++) {
        ...
        // 与所有已放置团块的最小间隙
        // 有机多边形顶点半径可达 1.05×R（0.7+0.16+0.11+0.08），按 1.05R 计算防重叠
        let minGap = Infinity;
        for (const p of placed) {
          const dist = Math.sqrt((candX - p.cx) ** 2 + (candZ - p.cz) ** 2);
          const gap = dist - (R + p.R) * 1.05;
